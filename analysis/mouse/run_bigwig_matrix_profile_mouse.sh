...
# Directories
BAMDIR="Drp1_mouse_dge"
OUTDIR="plot_coverage_mouse"
# GTF="ref/DNM1L.gtf"
GTF="temp_ref/NM_001360007.gtf" # this one contains all exons
...
for BAM in "$BAMDIR"/*.bam; do
    ...
    if [[ ! -f "$BIGWIG" ]]; then
        ...
        bamCoverage -b "$BAM" -o "$BIGWIG" --normalizeUsing None --binSize 1 -p 16 --region NC_000082.7
    else
