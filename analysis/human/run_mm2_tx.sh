...
tx='/vast/projects/lab_davidson/yan.a/ref/ncbi/human_110/GCF_000001405.40_GRCh38.p14_rna.fna.gz' ## 110 version

# Create minimap2 index if it doesn't exist
minimap2 -x map-ont -I 1000G -t $num_cores -d $outdir/tx.mmi $tx

tx_index="$outdir/tx.mmi"
input_folder="Drp1_human_dge"

for raw_reads in ${input_folder}/*.bam; do
    name=$(basename "$raw_reads" .bam)
    out_fastq="$outdir/${name}.fastq"
    out_bam="${outdir}/${name}_sorted.bam"

    # Filter to chr12 region and convert to FASTQ in one pipe
    samtools view -@ $num_cores -b "$raw_reads" chr12:32660000-32800000 | \
