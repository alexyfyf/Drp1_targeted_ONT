...
bam.files <- list.files(args[1], pattern = '_sorted_dedup_merged.bam$', full.names = TRUE)
bam.files <- normalizePath(bam.files)
length(bam.files)

# gtf.file <- list.files(paste0(args[1], '/ref'), pattern = 'gtf$', full.names = TRUE)
# fa.file <- list.files(paste0(args[1], '/ref'), pattern = 'fa$', full.names = TRUE)

if (grepl('human', args[1])) {
    gtf.file <- "/home/users/allstaff/yan.a/davidson_longread/yan.a/20240326_stVincent_Max_MitocDNAPool/ncbi/dedup/temp_ref/GCA_000001405.15_GRCh38_full_analysis_set.refseq_annotation.gtf"
    fa.file <- "/home/users/allstaff/yan.a/davidson_longread/yan.a/20240326_stVincent_Max_MitocDNAPool/ncbi/dedup/temp_ref/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna"
